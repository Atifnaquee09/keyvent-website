...
const MakeoverPage = () => {
  ...
  useEffect(() => {
    const fetchMakeoverArtists = async () => {
      try {
        setLoading(true);
        setError('');
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch makeover artists: ${response.status} ${response.statusText}`);
        }
        
        const result = await response.json();
        
        if (result.success && Array.isArray(result.makeoverArtists)) {
          setMakeoverArtists(result.makeoverArtists);
        } else {
          throw new Error('Invalid data format received');
        }
      } catch (error) {
        console.error('Error fetching makeover artists:', error);
        setError('Failed to load makeover artists. Please try again later.');
      } finally {
        setLoading(false);
      }
    };

    fetchMakeoverArtists();
  }, []);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg font-medium text-gray-700">Loading makeover artists...</p>
        </div>
      </div>
    );
  }
      ...
      <section className="relative py-16 xs:py-12 sm:py-20 md:py-24 bg-gradient-to-r from-purple-600 to-pink-500 text-white overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="font-playfair text-4xl xs:text-3xl sm:text-5xl md:text-6xl font-bold mb-6">
            Professional Makeover Artists
          </h1>
          <p className="font-inter text-xl xs:text-lg sm:text-2xl max-w-3xl mx-auto mb-8">
            Discover our talented team of makeover artists who enhance your beauty with creativity and expertise
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        {makeoverArtists.length > 0 ? (
          <div className="grid grid-cols-1 xs:grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {makeoverArtists.map((artist) => (
              <div 
                ...
                onClick={() => navigate(`/services/makeover/${artist._id || artist.id}`)}
              >
          ...
          <div className="text-center py-12">
            <h3 className="text-xl font-medium text-gray-700 mb-2">No makeover artists available</h3>
            <p className="text-gray-500">Check back later for our talented makeover artists.</p>
          </div>
        )}
      </div>
    </div>

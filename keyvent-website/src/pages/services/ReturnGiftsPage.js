    ...
    const fetchReturnGifts = async () => {
      try {
        ...
        if (result.success && Array.isArray(result.returnGifts)) {
          setReturnGifts(result.returnGifts);
        } else {
          throw new Error('Invalid data format received');
        }
      } catch (error) {
        console.error('Error fetching return gifts:', error);
        setError('Failed to load return gifts. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg font-medium text-gray-700">Loading return gifts...</p>
        </div>
      </div>
    );
  }
      ...
      <section className="relative py-16 xs:py-12 sm:py-20 md:py-24 bg-gradient-to-r from-purple-600 to-pink-500 text-white overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="font-inter text-xl xs:text-lg sm:text-2xl max-w-3xl mx-auto mb-8">
            Discover our exquisite collection of return gifts perfect for your special occasions
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        {returnGifts.length > 0 ? (
          ...
          <div className="text-center py-12">
            <h3 className="text-xl font-medium text-gray-700 mb-2">No return gifts available</h3>
            <p className="text-gray-500">Check back later for our beautiful return gift collections.</p>
          </div>
        )}
      </div>
    </div>

...
const AddDecoratorPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!decoratorData.name.trim()) {
        throw new Error('Decorator name is required');
      }

      if (!decoratorData.profileImage) {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || `Failed to add decorator: ${response.status} ${response.statusText}`);
      }

      const result = await response.json();
      console.log('Decorator added successfully:', result);
      
      // Navigate back to admin dashboard
      navigate('/admin');
    } catch (error) {
      console.error('Error adding decorator:', error);
      setError(error.message || 'Failed to add decorator');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
          <div className="bg-gradient-to-r from-purple-600 to-pink-500 px-6 py-8 sm:px-10">
            <h1 className="text-3xl font-bold font-playfair text-white">Add New Decorator</h1>
            <p className="mt-2 text-purple-100">Create a new decorator profile with images</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8 p-6 sm:p-10">
            ...
            <div className="bg-purple-50 p-6 rounded-2xl shadow-lg border border-purple-100">
              ...
              <div className="grid grid-cols-1 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Decorator Name</label>
                  <input
                    type="text"
                    name="name"
                    value={decoratorData.name}
                ...
                <div>
                  ...
                  {decoratorData.profileImage && !profileImagePreview && (
                    <div className="mt-2 w-32 h-32 rounded-md overflow-hidden border">
                      <img 
                        src={`${process.env.REACT_APP_SERVER_URL || 'https://api.keyvent.in'}${decoratorData.profileImage}`} 
                        alt="Profile" 
                        className="w-full h-full object-cover"
                        onError={(e) => {
                  ...
                  <p className="mt-1 text-sm text-gray-500">Upload a profile image for this decorator</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-pink-50 p-6 rounded-2xl shadow-lg border border-pink-100">
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary"
                />
                <p className="mt-1 text-sm text-gray-500">Select multiple images to upload for this decorator's collection</p>
              </div>

              {decoratorData.images.length > 0 && (
                <div className="mt-6">
                  <h3 className="text-lg font-medium text-gray-900 mb-4">Uploaded Images ({decoratorData.images.length})</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {decoratorData.images.map((image, index) => (
            ...
            <div className="flex justify-end">
              ...
              >
                {loading ? 'Adding Decorator...' : 'Add Decorator'}
              </button>
            </div>
          </form>
        </div>
...
export default AddDecoratorPage;